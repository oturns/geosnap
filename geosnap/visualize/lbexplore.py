...
warn(
    "This geopandas extension is experimental and untested; it will soon be replaced by "
    + "another implementation in `lonboard`. See <https://github.com/developmentseed/lonboard/pull/860>",
    stacklevel=2
)
...
class LonboardAccessor:
    ...
    def explore(  # noqa: C901, PLR0912, PLR0913, PLR0915
        ...
        *,
        ...
        layer_kwargs: (
            ScatterplotLayerKwargs | PathLayerKwargs | PolygonLayerKwargs | None
        ) = None,
        ...
        if any(["LineString", "MultiLineString"]) in gdf.geometry.geom_type.unique():
            line = True
        if color:
            if line:
        ...
        if isinstance(elevation, str):
            if elevation in gdf.columns:
                elevation: pd.Series = gdf[elevation].values
            else:
                raise ValueError(
        ...
        if elevation is not None:
            ...
            if ["Polygon", "MultiPolygon"] in gdf.geometry.geom_type.unique():
                ...
                raise ValueError(
                    "Point and Line-based geometries cannot be extruded. Try buffering to convert to polygons instead"
                )
        new_m: Map = viz(
            gdf,
