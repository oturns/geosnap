...
def KMeansbyAllMetro(nClusters, years, codestring, allcodestring, control, logfileName):
	...
	#metroDic = {}        # key: metroid,  value: ('stateid':  'state':  'metroname':  'geojson':  'countyids': [...]}
	for mdx, mKey in enumerate(metroKeys):
			if (len(metroKeys) > 100 and (mdx % int(len(metroKeys) / 100)) == 0):
				now = datetime.now()
				now_str = now.strftime('%Y-%m-%d')+' '+ now.strftime('%H:%M:%S')
				p = open(logfileName, 'a')
				p.write(str(os.getpid())+' progress '+now_str+' '+str(mdx)+'/'+str(len(metroKeys))+'\n')
				p.close()
	...
	now_str = now.strftime('%Y-%m-%d')+' '+ now.strftime('%H:%M:%S')
	p = open(logfileName, 'a')
	p.write(str(os.getpid())+' progress '+now_str+' '+str(len(metroKeys))+'/'+str(len(metroKeys))+'\n')
	p.close()
...
def getParameter(argv):
	...
	try:
		...
		print("CheckProgressNeighborhoodAnalysis.py -y <year> -i <inputfile>")
		sys.exit(2)
	for opt, arg in opts:
		if opt == "-h":
			print("CheckProgressNeighborhoodAnalysis.py -y <year> -i <inputfile>")
...
# CheckProgressNeighborhoodAnalysis.py -y 1980 -i "C:\Users\Administrator\Documents\2018-01-30 LTDB setup\LongitudinalNeighborhoodAnalysis_SelectedVariableList.txt"

	#started_datetime = datetime.now()
	#dateYYMMDD = started_datetime.strftime('%Y%m%d')
	#timeHHMMSS = started_datetime.strftime('%H%M%S')
	#print 'CheckProgressNeighborhoodAnalysis start at %s %s' % (started_datetime.strftime('%Y-%m-%d'), started_datetime.strftime('%H:%M:%S'))
	...
	if (sOpt == 'metros'):
		...
		logfileName = path + '/byMetro' + '.log'
		#print(logfileName)
		if (os.path.isfile(logfileName)):
			f = open(logfileName, 'r')
			log_data = f.readlines()
			f.close()
			for aline in log_data:
				if (aline.endswith('\n')): out_data.append(aline[0:-1])
				else:                      out_data.append(aline)
	...
	#print 'CheckProgressNeighborhoodAnalysis ended at %s %s    Elapsed %02d:%02d:%02d' % (ended_datetime.strftime('%Y-%m-%d'), ended_datetime.strftime('%H:%M:%S'), hours, minutes, seconds)
