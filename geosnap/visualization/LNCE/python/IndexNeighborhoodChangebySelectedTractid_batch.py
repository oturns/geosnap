...
def KMeansbySelectedTractid(nClusters, years, stateid, metroid, countyid, codestring, allcodestring, control):
	...
	#print(works)
	for w, row in enumerate(works):
		print("KMeans:", row, works[row])
	...
	for y, row in enumerate(yearData):
		print("yearData" + str(y) + ":", row)
...
def KMeansbyAllMetro(nClusters, years, codestring, allcodestring, control):
	...
	path = '../cache/metro_' + str(nClusters) + '_' + '_'.join(year for year in years) + '_' + control['bitstring']
	...
	for mdx, mKey in enumerate(metroKeys):
		...
		incAve 	  = metroDic[mKey]['incAve'];
		record = (mKey, metroname, geojson, incAve)
		oCount += 1
		#if (oCount > 5): break
		output.append(record)
