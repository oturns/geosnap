...
def test_multigroup_tempdyn():
    ...
    assert_array_almost_equal(df.values, np.array(
      [[0.2887, 0.3492, 0.3639],
       ...
       [0.2697, 0.3846, 0.4693]]), decimal=2)


def test_singlegroup_tempdyn():

    df = singlegroup_tempdyn(gdf.copy(), group_pop_var='n_nonhisp_black_persons', total_pop_var='n_total_pop')
    assert_array_almost_equal(df.values, np.array(
      [[0.9727, 0.9803, 0.9715],
       ...
       [0.0557, 0.046 , 0.0485],
       [1.0136, 1.0067, 1.016 ]]), decimal=2)


def test_spacetime_dyn():

    df = spacetime_dyn(gdf.copy(), segregation_index=Entropy, group_pop_var='n_nonhisp_black_persons', total_pop_var='n_total_pop',distances= range(1000,2000, 1000))
    assert_array_almost_equal(df.values, np.array(
      [[0.07684793, 0.05255198, 0.11470159],
       [0.07635796, 0.05217017, 0.10912089]]), decimal=2)
